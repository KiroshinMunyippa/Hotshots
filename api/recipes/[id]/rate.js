...
export default async function handler(req, res) {
  ...
  const { error } = await appData.from('recipe_ratings').upsert({
    recipe_id: recipeId, user_id: user.id, rating: value, updated_at: new Date().toISOString()
  });
  if (error) return res.status(500).json({ error: error.message });

  const { data: ratings, error: fetchError } = await appData.from('recipe_ratings').select('rating').eq('recipe_id', recipeId);
