...
const state = {
  tab: 'summary', filter: 'All', recipeId: null, authMode: 'signin', authError: '', authModalOpen: false, upgrading: false,
  ...
  session: null, profile: null, userRatingCache: {}, drinks: [], inventory: [], loading: false
};
const isGuest = () => Boolean(state.session?.user?.is_anonymous);
...
const avgScore = (id) => { const r = ratingFor(id); return r.count ? r.total / r.count : 0; };
const ratingText = (id) => { const rating = ratingFor(id); return rating.count ? `${avgScore(id).toFixed(1)}/5 HotShot Score · ${rating.count} ${rating.count === 1 ? 'rating' : 'ratings'}` : 'No HotShot Score yet'; };
const ratingShort = (id) => { const rating = ratingFor(id); return rating.count ? `${avgScore(id).toFixed(1)} (${rating.count})` : 'New'; };
function hotshotIcon(score, size = 17) {
  const ratio = Math.max(0.12, Math.min(1, score / 5));
  const color = `hsl(${30 - ratio * 14}, ${68 + ratio * 22}%, ${42 + ratio * 14}%)`;
  return `<svg class="hotshot-icon" viewBox="0 0 24 30" width="${size}" height="${Math.round(size * 1.25)}" aria-hidden="true">
    <path d="M3.5 3 L20.5 3 L13.6 23.5 C13.2 25.5 10.8 25.5 10.4 23.5 Z" fill="none" stroke="currentColor" stroke-opacity=".55" stroke-width="1.6" stroke-linejoin="round"/>
    <g style="transform-origin:12px 22px; transform: scaleY(${ratio.toFixed(2)})">
      <path d="M12 7 C9.5 11 8 13 8.8 16.5 C9.3 19 10.6 20.5 12 20.5 C13.4 20.5 14.7 19 15.2 16.5 C16 13 14.5 11 12 7 Z" fill="${color}" opacity="${(0.55 + ratio * 0.45).toFixed(2)}"/>
    </g>
  </svg>`;
}
...
async function loadInventory() {
  if (isGuest()) { state.inventory = readStore('guest-inventory', []).map(item => ({ ...item, ...ingredientTypeMeta(item.type) })); return; }
...
async function loadProfile() {
  if (isGuest()) { state.profile = null; return; }
...
function ratingControl(drink) {
  if (isGuest()) return `<p class="guest-rating-note">Create an account to give a HotShot Score</p>`;
...
function drinkCard(drink, variant = 'rail', showRemove = false) {
  if (variant === 'discover') return `
    <article class="discover-card recipe-card" data-view-recipe="${drink.id}">
      ...
      <small class="rating-summary">${hotshotIcon(avgScore(drink.id))} ${ratingText(drink.id)}</small>
  ...
  return `
    <article class="drink-card">
      ...
      <div class="drink-card-footer"><span class="community-rating">${hotshotIcon(avgScore(drink.id))} ${ratingShort(drink.id)}</span><button class="like ${drink.favourite ? 'is-favourite' : ''}" data-favourite="${drink.id}" aria-label="Toggle favourite">${drink.favourite ? '♥' : '♡'}</button></div>
    </article>`;
}
...
function renderAuth() {
  ...
  view.innerHTML = `
    <p class="eyebrow">${upgrading ? 'Keep your recipes' : isSignup ? 'Create your account' : 'Welcome back'}</p>
    <h1 class="page-title">${upgrading ? 'Make it<br>official.' : isSignup ? 'Join<br>HotShots.' : 'Sign in to<br>your bar.'}</h1>
    <p class="page-subtitle">${upgrading ? 'Add an email and password and the recipes you already made as a guest come with you.' : isSignup ? 'Create an account to save recipes, ratings, and your bar across devices.' : 'Sign in to pick up your recipes and bar where you left off.'}</p>
    <form class="form-card" id="auth-form">
      ...
      <button class="primary-button" type="submit">${upgrading ? 'Create account' : isSignup ? 'Create account' : 'Sign in'}</button>
    </form>
    ${upgrading ? `<button class="text-link" id="cancel-upgrade" type="button" style="margin-top:14px">Keep browsing as a guest</button>` : `
    ...
    ${state.authModalOpen ? `
      <div class="modal-backdrop">
        <div class="modal-card">
          ...
          <p>That email and password didn't match an account — it might not exist yet, or the password could be wrong. Want to create an account, or just explore as a guest for now?</p>
          <button class="primary-button" data-modal-action="create">Create an account</button>
          <button class="secondary-button" data-modal-action="guest">Continue as guest</button>
          <button class="text-link" data-modal-action="dismiss" style="margin-top:10px">Try signing in again</button>
        </div>
      </div>` : ''}`;
}
...
function renderRecipeDetail() {
  ...
  view.innerHTML = `
    ...
    <section class="recipe-detail">
      ...
      <div class="recipe-rating"><span>${hotshotIcon(avgScore(drink.id))} ${ratingText(drink.id)}</span>${ratingControl(drink)}</div>
...
function renderAccount() {
  ...
  if (isGuest()) {
    view.innerHTML = `
      ...
      <p class="page-subtitle">Your bar, favourites, and log stay on this device. Recipes you create are already saved — create an account to keep everything and rate recipes too.</p>
...
async function rateDrink(id, value) {
  if (isGuest()) return showToast('Create an account to give a HotShot Score.');
  ...
  try {
    ...
    render(); showToast(`You gave this a HotShot Score of ${value}`);
  } catch (err) { showToast(err.message); }
}
...
document.addEventListener('click', event => {
  ...
  if (remove) {
    ...
    if (isGuest()) { state.inventory = state.inventory.filter(i => i.id !== id); saveGuestInventory(); renderBar(); return showToast(`${item?.name || 'Item'} removed from your bar`); }
  ...
  if (event.target.closest('#reset-local')) {
    const message = isGuest()
      ? 'Clear favourites, your drink log, and your bar on this device? Recipes you created stay saved.'
    ...
    if (isGuest()) { state.inventory = []; saveGuestInventory(); }
    renderAccount(); showToast('Device data cleared');
    return;
  }
...
document.addEventListener('submit', event => {
  if (event.target.id === 'auth-form') {
    ...
    supabase.auth.signInWithPassword({ email, password }).then(({ error }) => {
      if (error) { state.authError = error.message; state.authModalOpen = true; return renderAuth(); }
    });
    return;
  }
...
supabase.auth.onAuthStateChange((_event, newSession) => {
  ...
  if (newSession && !wasSignedIn) return bootstrapData();
  if (!newSession) { state.drinks = []; state.inventory = []; state.profile = null; state.tab = 'summary'; state.upgrading = false; render(); }
});
